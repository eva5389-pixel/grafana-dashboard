...
def main() -> int:
    ...
    robotics_path = Path(__file__).resolve().parents[1] / "config" / "robotics_holdings.json"
    for holding in json.loads(robotics_path.read_text(encoding="utf-8")):
        item = next((candidate for candidate in raw if candidate["name"] == holding["name"]), None)
        if item is None:
            continue
        return_1y = number(item.get("return_1y"))
        ...
        score_value = (return_1y + momentum + sharpe_value / 5 + theme_score
                       if None not in (return_1y, momentum, sharpe_value) else None)
        grouped["robotics"].append({
            "category": "robotics", "category_name": categories["robotics"]["name"],
            "name": item["name"], "moneydj_id": "", "twelve_data_symbol": "",
            "benchmark": categories["robotics"]["benchmark_symbol"],
            "return_1y": return_1y, "benchmark_return_1y": None,
            "excess_return_1y": None, "momentum_6m": momentum,
            "sharpe": sharpe_value, "max_drawdown": None, "recovery_days": None,
            "score": score_value,
            "signal": "買進" if score_value is not None and score_value >= 2 else "觀察",
            "robotics_coverage": holding["coverage"],
            "robotics_exposure": holding["exposure"],
            "robotics_holdings": holding["holdings"],
            "robotics_as_of": holding["as_of"],
            "status": f"已驗證機器人供應鏈持股；Benchmark資料不足；{holding['source']}",
        })
    ...
    fields = ["rank", "category_name", "name", "moneydj_id", "twelve_data_symbol", "benchmark",
              ...
              "memory_exposure", "memory_holdings", "memory_as_of", "robotics_coverage",
              "robotics_exposure", "robotics_holdings", "robotics_as_of", "status"]
    ranked = []
    ...
    for category, meta in categories.items():
        ...
        for rank, row in enumerate(rows[:10], 1):
            ...
            formatted["robotics_exposure"] = fmt(row.get("robotics_exposure"), percent=True)
